...
def polynomial_test(G: Automata_t, target: Event) -> bool:
    ...
    for g in scc:
        for v in g:
            if len(g) == 1 and is_self_loop(ver,v) == False:
                continue
            else:
                event = str(v["out"][0][1])
                i = event.find(',')
...
def is_self_loop(G,vertex):
    vertices = [v for v in G.vs]
    try:
        out = vertex["out"][0][0]
    except IndexError:
        return False
    if vertex == vertices[out]:
        return True
    return False
...
class tarjans_algorithm:
    ...
    def DFS(self, v, name = False) -> list:
        """
        ...
        if name == False:
            ...
            self.DFSUtil(v, visited, d, True)
        return d

    def DFSUtil(self,v,visited,d, name = False):
        if name == False:
            d.append(v)
        else:
            d.append(v["name"])
            ...
            if neighbour not in visited:
                if name == False:
                    self.DFSUtil(neighbour,visited,d)  
                     ...
                     self.DFSUtil(neighbour,visited,d, True)  
...
class johnsons_algorithm:
    ...
    def simple_cycles(self,G) -> list:
        ...
        DFS_names = initial.DFS(G.vs[0],True)
        ...
        while(start_index <= (len(self.vertices)-1)):
            ...
            if(least_vertex[0] != None):
                ...
                start_index += 1#DFS_names.index(least_vertex[0]["name"]) + 1
            else:
                break
        ...
        res = []
        for cycles in self.all_cycles:
            fill = []
            for v in cycles:
                fill.append(v["name"])
            res.append(fill)
        final = [i for n, i in enumerate(res) if i not in res[:n]]
        return final
...
def extended_diagnoser(G:Automata_t, target: Event)->Automata_t:
    ...
    Ext_diag_edges = []  
    queue = deque([(prime_x0, Ext_diag_vertices[0]["name"])])
    while len(queue) > 0:
        x1 = queue.popleft()
        cur_name = x1[1]
    ...
    ext_diag.events = prime.events
    ext_diag.Euc.update(prime.Euc)
    ext_diag.Euo.update(prime.Euo)

    return ext_diag

def prime_automata(G: Automata_t, target: Event) -> Automata_t:
    A_label = DFA()
    A_label.add_vertices(2, names = ["N","Y"])
    A_label.add_edges([(0,1),(1,1)], labels = [target, target])
    A_label.Euo = {target} 
    ...
    while len(queue) > 0:
        ...
        for e in active_x1:
            ...
            if e not in unobservable: #if e is observable
                obs_states = [(GparA.vs[active_x1[e]],e)]
            elif e in unobservable:
                try:
                    GparA.vs[active_x1[e]]["unobs_visit"]
                except:
                    obs_states = BFS_search(GparA, unobservable, GparA.vs[active_x1[e]],result)
                else:
                    if GparA.vs[active_x1[e]]["unobs_visit"] == False:
                        obs_states = BFS_search(GparA, unobservable, GparA.vs[active_x1[e]],result)
            else: 
                continue
    ...
    prime.add_vertices(
        ...
        [v["unobs_visit"] for v in prime_vertices],
    )
    prime.add_edges(
        [e["pair"] for e in prime_edges], [e["label"] for e in prime_edges]
    )   

    prime.events = GparA.events
    prime.Euc.update(GparA.Euc)
    prime.Euo.update(GparA.Euo)

    return prime

def BFS_search(G:Automata_t, unobservable:list, V, result) -> list:
    vertices = [v for v in G.vs]
        ...
        for e in active_x1:
            ...
            if visited[v] == False:
                ...
                if e not in unobservable:
                    result.append((vertices[v],e))
                else:
                    G.vs[v]["marked"] = True
                    BFS_search(G, unobservable, vertices[v], result)
        return result
