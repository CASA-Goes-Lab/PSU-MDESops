            ...
            for e in active_events:
                ...
                for x1_dst in x1_outs:
                    for x2_dst in x2_outs:
                        dst_index = (x1_dst.index, x2_dst.index)
                        dst_state = G_out.vs.find(indexes_eq=dst_index)
                        G_out.add_edge(x.index, dst_state.index, e, fill_out=True)
    ...
    for G2 in input_list:
        ...
        for x in G_out.vs:
            ...
            for e in x1_ex:
                x1_outs = {G1.vs[out[0]] for out in x1["out"] if out[1] == e}
                for x1_dst in x1_outs:
                    dst_index = (x1_dst.index, x2.index)
                    dst_state = G_out.vs.find(indexes_eq=dst_index)
                    G_out.add_edge(x.index, dst_state.index, e, fill_out=True)
            ...
            for e in x2_ex:
                x2_outs = {G2.vs[out[0]] for out in x2["out"] if out[1] == e}
                for x2_dst in x2_outs:
                    dst_index = (x1.index, x2_dst.index)
                    dst_state = G_out.vs.find(indexes_eq=dst_index)
                    G_out.add_edge(x.index, dst_state.index, e, fill_out=True)
