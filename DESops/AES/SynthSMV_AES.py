...
def create_SMV_file(G,model_fname):
	...
	with open(model_fname, 'w') as f:
		...
		for v in g.vs:
			...
			for a in action_set:
					if a in G.Euo and a in G.Euc:
						source_state = [e.source for e in g.es(_target = v.index) if e["label"]== a]
						print(source_state)
						if source_state:
							print('\t\t!nX[{0}] & !asg & !obs & (nX[{1}]'.format(v.index,source_state[0]),end = "", file = f)
						...
						if source_state:
							...
							for j in range(len(source_state)-1):
								print(' | nX[{0}]'.format(source[j+1]),end="",file = f)
							print(') & gamma[{0}] & !asg & !obs: TRUE;'.format(ctr.index(a)), file = f)
		...
		for i in range(g.vcount()):
			print(' & uoreach.nX[{0}] = next(uoreach.nX[{0}])'.format(i),end="",file = f)
		print(': TRUE;',file=f)
		print('\t\tobs & events_o!= eps: FALSE;', file=f)
		print('\t\tobs & events_o = eps: TRUE;',file=f)
		print('\t\tTRUE: FALSE;',file=f)
		...
		for v in g.vs:
			...
			for a in obs:
					if a in action_set:
						...
						# print(source_state)
						if source_state:
							print('\t\t!asg & obs & (Q[{0}]'.format(source_state[0]),end = "", file = f)
							for j in range(len(source_state)-1):
								print(' | Q[{0}]'.format(source_state[j+1]),end="",file = f)
	...
	# RESTRICTION BASED ON ENABLED EVENTS AND ON CURRENT STATE ESTIMATE
		...
		for v in lst:
			...
			for i in range(len(v)):
				...
				if i!=len(v)-1:
					print('gammas[{0}]={1} & '.format(i,TRUE_FALSE(v[i])),end="",file=f)
				else:
					print('gammas[{0}]={1} -> events_o in '.format(i,TRUE_FALSE(v[i])),end="",file=f)
		...
		j = 0
		for v in g.vs:
			action_set = [e["label"] for e in g.es(_source = v.index) if e["label"] in obs]
			action_set.append('eps')
			# print(action_set)
			print('(Q[{0}] -> events_o in {{{1}}})'.format(v.index,','.join(action_set)),end="",file=f)
			if j!=len(g.vs)-1:
				print(' | ',end="",file = f)
			else:
				print(') & ',file=f)
		...
		for i in range(len(ctr)):
			if i!=len(ctr)-1:
				print('gammas[{0}] = TRUE |  gammas[{0}] = FALSE | '.format(i),end="",file = f)
			else:
