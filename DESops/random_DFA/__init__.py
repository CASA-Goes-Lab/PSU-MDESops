...
if not os.path.isfile(rand_DFA_dir):
    sys.exit("Could not find random_DFA executable. See instructions to install in DESops/random_DFA/regal_readme.txt")

#output = subprocess.run(["./random_DFA", str(num_vert), str(size_alphabet), str(num_automata)], capture_output=True, text=True)

#print(output.stdout)

def generate(num_vert, size_alphabet, g=None, timeout=None, Euc_p=0.5, Euo_p=0.5, overlap=True):
    this_dir = os.path.dirname(__file__)
    rand_DFA_dir = this_dir + "/regal-1.08.0929/random_DFA"
    output = subprocess.run([rand_DFA_dir, str(num_vert), str(size_alphabet), "1"], capture_output=True, text=True, timeout=timeout)
