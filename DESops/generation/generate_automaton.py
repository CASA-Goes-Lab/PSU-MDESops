...
from DESops.file.igraph_to_fsm import igraph_to_fsm
...
class FSM_Generator:
            ...
            if not self.g.vs[state].indegree():
                ...
                while True:
                    ...
                    if self.g.vs[okay_parent].outdegree() < self.max_trans_per_state:
                        ...
                        self.g.add_edge(good_parent, state, event)
                        break
                ...
                if good_parent is None:
                    ...
                    if event is not None:
                        if self.enforce_max_trans_per_state:
                            return False
                        self.max_trans_exceeded = False
                        self.g.add_edge(
                            okay_parent, state, event
                        )  # oh well, too many transitions ...
                    # else unable to make this accesible without violating determinism
    ...
    def write_files(self):
        ...
        if self.fsm_filename:
            igraph_to_fsm(self.fsm_filename, self.g, self.g.Euc, self.g.Euo)
